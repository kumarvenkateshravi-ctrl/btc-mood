...
// WatchlistRowView renders a <tr>; wrap it so the markup nests validly.
const renderRow = (r: WatchlistRow, active: boolean) =>
  renderToStaticMarkup(<table><tbody><WatchlistRowView row={r} active={active} onSelect={() => {}} /></tbody></table>);
...
describe('WatchlistRowView', () => {
  ...
  it('a negative row uses the bear tone and a leading minus', () => {
    const html = renderRow(row({ symbol: 'ETHUSDT', label: 'ETHUSDT', chg: -12.56, chgPct: -0.31 }), false);
