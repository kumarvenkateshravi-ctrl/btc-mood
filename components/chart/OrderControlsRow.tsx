...
interface OrderControlsRowProps {
  ...
  hasTp: boolean;
  hasSl: boolean;
  /** 'staged' (default): full row — ⇅/Discard/Confirm + TP/SL toggles, for a
   *  pending order before Confirm. 'position': reduced row — TP/SL add-chips
   *  only, for an open position that has no tp/sl yet (TV's dotted "TP SL"
   *  chips). */
  mode?: 'staged' | 'position';
  onReverse?: () => void;
  onDiscard?: () => void;
  onConfirm?: () => void;
  onToggleTp: () => void;
...
/** TV-style staged-order / open-position controls, docked to the entry line's y. */
export function OrderControlsRow(p: OrderControlsRowProps) {
  ...
  const isStaged = (p.mode ?? 'staged') === 'staged';
