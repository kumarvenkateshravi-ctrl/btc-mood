...
describe('Phase 1c decideWithSmc', () => {
  it('smc disabled ⇒ identical to the engine called without smc', () => {
    ...
    expect(decideWithSmc(intel, candles, { '1d': smcSnapshot }, false))
      .toEqual(computeTradeDecision(intel, candles));
  });

  it('smc enabled ⇒ the EXECUTION timeframe snapshot is applied', () => {
    const intel = readyIntel(); // hier() default: no perTimeframe → executionTf = controller '1d'
    const candles = { '1d': bars(LONG) } as const;
    const d = decideWithSmc(intel, candles, { '1d': smcSnapshot }, true);
    ...
    const ignored = decideWithSmc(intel, candles, { '15m': smcSnapshot }, true);
